/*
	...
	Description:
		creates the 'Sparks' on top of Twins
*/

if (!hasInterface) exitWith {};

params ["_obj_emit","_paz_emit"];

_sparky_sun = ["spark1","spark2","spark5","spark4"] call BIS_fnc_selectRandom;
//[_obj_emit,[_sparky_sun,300]] remoteExec ["say3d"];

_spark_type = ["white","orange"] call BIS_fnc_selectRandom;
//if (_repeat isEqualTo "spark3") then {_spark_type isEqualTo "white"};

_drop = 0.001+(random 0.05);
_scantei_spark = "#particlesource" createVehicleLocal (getPosATL _obj_emit);

if (_spark_type isEqualTo "orange") then 
{
