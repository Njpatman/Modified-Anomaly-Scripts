...
_poz_ini = [_mark_orig select 0,_mark_orig select 1,round (2+random 7)];
//_obiect_orb = "Sign_Sphere25cm_F" createVehiclelocal [_mark_orig select 0, _mark_orig select 1,round (2+random 7)];
_obiect_orb = "Sign_Sphere10cm_F" createVehiclelocal [_mark_orig select 0, _mark_orig select 1,round (2+random 7)];
...
while {(player distance _ob_slide) < 2000} do 
{
	...
	_sunet_spark = ["sparks_1","sparks_2"] call BIS_fnc_selectRandom;
...
if ((count _list_units_in_range>0) and ((player distance _obiect_orb)< 2000)) then 
{
	...
	{/*to do*/} 
		...
		{
		nul=[_voice_orb,_voice_orb] execVM "NJ_Spark\al_spark_attack.sqf";
		[[_x],"NJ_Spark\al_spark_damage.sqf"] remoteExec ["execVM",2];
		};
